...
namespace SimpleProject.Controllers
{
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var result = _mapper.Map<List<GetCategoryListViewModel>>(categories);
            return View(result);
        }
        [HttpGet]
        ...
        public async Task<IActionResult> Update(UpdateCategoryViewModel model)
        {
            if (ModelState.IsValid)
            {
                var oldCategory = await _categoryService.GetCategoryByIdWithouIncludeAsync(model.Id);
                if (oldCategory == null)
                {
                    return NotFound();
                }
        ...
        public async Task<IActionResult> DeleteConfirm(int? id)
        {
            ...
            var category = await _categoryService.GetCategoryByIdWithouIncludeAsync((int)id);
            if (category == null)
            {
                return NotFound();
            }
